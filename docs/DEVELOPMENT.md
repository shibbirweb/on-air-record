...
For how the application is used rather than how it is built, see the
[user guide](USER_GUIDE.md). Its screenshots are regenerated by hand against a seeded demo instance, so a
change to the interface means a change there too.
