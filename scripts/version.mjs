...
/**
 * A section of the changelog, without its heading. With no version, the `[Unreleased]` section: what a
 * beta is published with. With one, like 0.6.0, that released version's section: what release.yml writes
 * onto a stable release whose notes were left empty or generated by GitHub.
 */
function commandNotes(version) {
  ...
  if (start === -1) {
    console.error(`CHANGELOG.md has no [${label}] section to take release notes from`);
    return 1;
  }
  ...
  if (notes === '') {
    console.error(`the [${label}] section of CHANGELOG.md is empty`);
    return 1;
  }
  console.log(notes);
  return 0;
...
if (action === 'show' && argument === undefined) {
...
} else if (action === 'notes' && rest.length === 0) {
  process.exit(commandNotes(argument));
  ...
  console.error(
    [
      ...
      '  node scripts/version.mjs notes 0.6.0          print the section of a released version',
